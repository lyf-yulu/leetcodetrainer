...
- **报错随时辅导** → 编译错误、运行时异常、超时——任何一次运行（运行示例/调试/提交）
  只要出错，AI 就基于报错信息 + 你的代码 + 题面，**先判断你的思路对不对**：
  思路对 → 明确肯定「思路对了，只是这里写岔了」，只精准定位机械性小错；
  思路错 → 纠正方向。初学者不用等到「能通过」才有 AI 陪练
- **没思路就求助** → 第一次做题、完全没有头绪时，点「💡 求助」，
  AI 分阶段给提示（突破口 → 技巧/不变量 → 策略轮廓），一次只揭示一条，不直接给答案
- **错题本** → 每次提交失败自动归类错误类型，生成可复习的笔记
...
第一次做某道题、完全不知道从哪下手？点题目栏的「💡 求助」，AI 教练给出
**三条递进式提示**，并且**一次只显示一条**：

1. 温和的突破口 —— 重新框定问题或指出第一个有用的观察，不给技巧名
2. 该用的技巧或不变量 —— 如滑动窗口、前缀和、哈希表、双指针
3. 策略轮廓 —— 算法的大致形状，仍不写代码

每条之间你需要点「看提示 N」才会展开下一条——这是故意的，逼你在提示之间
自己动手试，而不是一次拿到全部答案。提示走到底后，卡片下方的「继续追问」
还能接着问。
...
public/
  ...
  ai.js                 教练提示词：通过/报错/诊断/补全用例/求助/追问/错题本
